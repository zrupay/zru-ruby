module ZRU
  ...
  class Paginator
    ...
    def initialize(json_dict, object_item_class, resource)
      if json_dict.is_a?(Hash)
        initialize_from_hash(json_dict, object_item_class, resource)
      elsif json_dict.is_a?(Array)
        initialize_from_array(json_dict, object_item_class, resource)
      else
        raise ArgumentError, 'json_dict must be a Hash or an Array'
      end
    end

    private

    def initialize_from_hash(json_dict, object_item_class, resource)
    ...
    def initialize_from_array(json_array, object_item_class, resource)
      @count = json_array.size
      @_previous = nil
      @_next = nil
      @results = []
      json_array.each do |result|
        @results.push(object_item_class.new(result, resource))
      end
      @resource = resource
    end
